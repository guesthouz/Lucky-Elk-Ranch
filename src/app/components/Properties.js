...
const Properties = ({ firebaseUid, apiUrl }) => {
  ...
  const sliderSettings = {
    ...
    slidesToScroll: 1,
    arrows: true,
    appendDots: (dots) => <ul style={{ position: 'absolute', bottom: '10px', left: '50%', transform: 'translateX(-50%)' }}>{dots}</ul>,
    prevArrow: <PrevArrow />,
    nextArrow: <NextArrow />,
  };
      ...
      {properties.map((property) => (
        <Grid item xs={12} sm={6} key={property._id}>
          <Card className="card">
            ...
            <CardContent>
            <Typography className='type-label'>
                {property.type}
            </Typography>
              <Typography style={{ marginBottom: '8px', marginTop:'10px', fontSize:'20px' }} variant="h5" component="div">
                {property.title}
              </Typography>
              <div style={{ display: 'flex', alignItems: 'center', marginBottom: '10px' }}>
                <FmdGoodIcon style={{ marginRight: '5px', marginLeft: '-3px', color: 'rgb(204, 204, 204)', fontSize: '16px' }} />
              ...
              <Grid container spacing={2} alignItems="center">
                ...
                <Grid item xs={6} sm={3} style={{ textAlign: 'right' }}>
                  <Typography variant="body1" style={{fontWeight:'600'}}>${property.pricing.price}/{property.pricing.label}</Typography>
                </Grid>
              </Grid>
            </CardContent>
          </Card>
